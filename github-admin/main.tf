  ...
  rules {
    ...
    dynamic "required_deployments" {
      for_each = {
        ...
        key => toset(value.required_deployments[each.value.branch])
        if value.name == each.value.repository
        && value.required_deployments != null
      }
      content {
        required_deployment_environments = each.value
      }
    }
    dynamic "required_status_checks" {
      for_each = {
        for key, value in var.repos[var.owner] :
        key => toset(value.required_status_checks[each.value.branch])
        if value.name == each.value.repository
